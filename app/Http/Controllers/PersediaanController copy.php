...
class PersediaanController extends Controller
{
    ...
    {   
        ...
        $sumPembelian = InvoiceAOPDetails::whereHas('header', function ($query) use ($tanggal_awal, $tanggal_akhir) {
                $query->whereBetween('billingDocument_date', [$tanggal_awal, $tanggal_akhir]);
            });

        $finalTotalPembelian = $sumPembelian->sum('amount_ex_disc');

        // $sumPembelian = InvoiceAOPDetails::whereBetween('crea_date', [$tanggal_awal, $tanggal_akhir]);
        // $finalTotalPembelian = $sumPembelian->sum('amount');
        
        $sumReturAOP = ReturAOPDetails::whereBetween('crea_date', [$tanggal_awal, $tanggal_akhir])->get();
        // $finalSumReturAOP = $sumReturAOP->sum('price');
        ...
        // dd($finalTotalReturAOP);

        $sumRetur = TransaksiReturDetails::whereBetween('crea_date', [$tanggal_awal, $tanggal_akhir]);
        $finalTotalRetur = $sumRetur->sum('nominal_total');

        $sumPenjualan = TransaksiInvoiceDetails::whereBetween('crea_date', [$tanggal_awal, $tanggal_akhir]);
        $finalTotalPenjualan = $sumPenjualan->sum('nominal_total');

        $sumPenjualanHet = TransaksiInvoiceDetailHet::whereBetween('crea_date', [$tanggal_awal, $tanggal_akhir]);
        ...
        $this->getPersediaan($tanggal_awal, $sumTotalPersediaan, $finalPersediaan, $finalPersediaanNext, $finalTotalPembelian, $finalTotalRetur, $finalTotalPenjualanHet, $finalTotalReturAOP);
        // $this->getPersediaanBulanan($tanggal_awal, $finalTotalPembelian, $sumRetur, $sumPenjualanHet);

        return view('persediaan.index',compact('persediaan', 'finalTotalReturAOP','finalPersediaan', 'sumTotalPersediaan', 'finalTotalPembelian', 'finalTotalRetur', 'finalTotalPenjualan', 'finalTotalPenjualanHet', 'persediaan_awal_index', 'bulanPilih', 'tahunPilih', 'persediaan_awal_index', 'tanggal_awal'));
    ...
    public function getPersediaan($tanggal_awal, $sumTotalPersediaan, $finalPersediaan, $finalPersediaanNext, $finalTotalPembelian, $finalTotalRetur, $finalTotalPenjualanHet, $finalTotalReturAOP){
        ...
        if ($checkBulan === null && $checkTahun === null) {
            $data['bulan'] = $bulan;
            $data['tahun'] = $tahun;
