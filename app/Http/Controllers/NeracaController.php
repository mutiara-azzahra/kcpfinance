...
class NeracaController extends Controller
{
    ...
    public function getNeraca(Request $request){
        ...
        //Pilih Bulan Tambah 1
        //$sliceTanggal = Carbon::createFromFormat('Y-m-d', $tanggal_awal);
        $pilihBulanTambah = $sliceTanggal->addMonth()->format('m');
        $getPilihanBulanTambah = 'saldo_bln'.$pilihBulanTambah;
        ...
        $neracaList = $saldoAwal->map(function ($saldo) use ($perkiraanNeraca, $getPilihanBulanTambah) {

            $perkiraan      = $saldo->perkiraan;
            $subPerkiraan   = $saldo->sub_perkiraan;
            $namaPerkiraan  = $saldo->nm_perkiraan;
            $saldoAwal      = $saldo->$getPilihanBulanTambah;
        ...
        foreach ($saldoAwalNeraca as $s){
            ...
            if (in_array($kodeL, $kodePerkiraanL)) {
                ...
                $sumSaldoAkhirL = $s['saldoAwal'];
            }

            $finalSumSaldoAkhirL += $sumSaldoAkhirL;
        }
        ...
        foreach ($neracaList as $s){
            ...
            if (in_array($kodeKewajiban, $kodePerkiraanKewajiban)) {
                ...
                $sumSaldoAkhirKewajiban = $s['saldoAwal'];
            }
        ...
        // sum aktiva
        $sumFinalAktiva = $finalSumSaldoAkhirKB + $finalSumSaldoAkhirD + $finalSumSaldoAkhirPU + $finalSumSaldoAkhirPL + $finalSumSaldoAkhirPersediaan + $finalSumSaldoAkhirPajak + $finalSumSaldoAkhirK + $finalSumSaldoAkhirPK + $finalSumSaldoAkhirL;

        // sum kewajiban
        $sumFinalKewajiban = $finalSumSaldoAkhirKewajiban + $finalSumSaldoAkhirHSP + $finalSumSaldoAkhirP ;
        ...
        'kodeL', 'kodePerkiraanL',
        'sumSaldoAkhirL', 'finalSumSaldoAkhirL',
